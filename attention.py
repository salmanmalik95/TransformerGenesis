...
class AttentionModel(BaseLanguageModel):
    """
    Neural network with self-attention mechanism.

    Architecture:
        Embeddings -> Q,K,V Projection -> Attention Scores -> Weighted Sum -> Output

    Self-attention allows each position to "look at" other positions
    and decide how much to weight each one based on content similarity.
    ...
    def initialize(self):
        """Initialize embeddings and attention weights."""
        ...
        # Attention projection matrices (single head)
        ...
        print(f"Initialized Q, K, V weights: {self.Wq.shape}")
    ...
    def forward(self, x_indices):
        ...
        """
        # Step 1: Get embeddings for context
        embeds = np.array([self.E[i] for i in x_indices])  # (context_size, embed_dim)

        # Step 2: Project to Q, K, V
        Q = np.dot(embeds, self.Wq)  # (context, dim)
        K = np.dot(embeds, self.Wk)
        V = np.dot(embeds, self.Wv)

        # Step 3: Compute attention scores
        # scores[i,j] = how much position i attends to position j
        scores = np.dot(Q, K.T)  # (context, context)

        # Step 4: Softmax to get attention weights
        weights = self.softmax_rows(scores)

        # Step 5: Weighted sum of values
        attended = np.dot(weights, V)  # (context, dim)

        # Step 6: Average attended representations
    ...
    def train(self, epochs=500, print_every=50):
        ...
        for epoch in range(epochs):
            ...
            for x, y in zip(self.X, self.Y):
                ...
                # Distribute gradient to embeddings
                for idx in x:
    ...
    def predict_next_word(self, context_words):
        """Predict next word with attention weights."""
...
if __name__ == "__main__":
    model = AttentionModel(context_size=2, embedding_dim=2, learning_rate=0.1)
    model.load_data()
    model.initialize()
